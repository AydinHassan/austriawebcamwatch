  ...
  <Card class="h-[500px] lg:h-full col-span-1 flex flex-col group relative">
    <CardHeader class="flex-row justify-between items-center absolute w-full hidden bg-background group-hover:flex z-1">
      ...
      <div class="flex gap-x-2">
        ...
        <span v-if="allowToggle" class="bg-secondary hover:bg-secondary/90 rounded p-0.5 cursor-pointer" @click="toggleWebcam(webcam.name)"><Cross2Icon></Cross2Icon></span>
        ...
        <Popover v-if="allowShare" @update:open="val => val && createShareLink([webcam])">
          <PopoverTrigger as-child>
            <span class="bg-secondary hover:bg-secondary/90 rounded p-0.5 cursor-pointer" variant="outline">
              <Share1Icon />
            </span>
          </PopoverTrigger>
